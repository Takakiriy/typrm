...
describe("string >>", () => {
    ...
    test("chageToAlphabets", () => {
        ...
        expect(lib.chageToAlphabets('かきくけこ さしすせそ たちつてと なにぬねの はひふへほ まみむめも やゆよ らりるれろ わをん'
            )).toBe('kakikukeko sasisuseso tatituteto naninuneno hahifuheho mamimumemo yayuyo rarirurero wawon');
        expect(lib.chageToAlphabets('きゃきゅきょ しゃしゅしょ ちゃちゅちょ にゃにゅにょ ひゃひゅひょ ふぁふぃふぇふぉ みゃみゅみょ りゃりゅりょ'
            )).toBe('kyakyukyo shashusho chachucho nyanyunyo hyahyuhyo fafifefo myamyumyo ryaryuryo');
        expect(lib.chageToAlphabets('がぎぐげご ざじずぜぞ だぢづでど ばびぶべぼ ぱぴぷぺぽ'
            )).toBe('gagigugego zajizuzezo dadidudedo babibubebo papipupepo');
        ...
        expect(lib.chageToAlphabets('っまっみっむっめっもっやっゆっよっいぇっらっりっるっれっろっぁっぃっぅっぇっぉ'
            )).toBe('mmammimmummemmoyyayyuyyoyyerrarrirrurrerrollallillullello');
        expect(lib.chageToAlphabets('っがっぎっぐっげっごっざっじっずっぜっぞっだっぢっづっでっどっばっびっぶっべっぼっぱっぴっぷっぺっぽ'
...
describe("file and file path >>", () => {
    ...
    ])("getGlobbyParameters >> %s", async (_caseName: string, arguments_: string, answerOfTarget: string, answerOfWildcard: string[]) => {
