...
export function  getStringBefore(input: string, keyword: string): string {
    ...
    for (const replacer of replacers) {
        ...
        if (replacer.fromCSV) {
            ...
            if (replacer.from) {
                ...
                var  replaceFrom: string | RegExp = keywords[keywords.length - 1];
            }
...
export const  timeFrames = new Map<string, TimeFrame>();

export function  get(label: string): TimeFrame {
    var  frame = timeFrames.get(label);
    if ( ! frame) {
        frame = new TimeFrame();
        frame.label = label;
        timeFrames.set(label, frame);
    }
    return  frame;
}

export function  getTimeFrames(labelPattern: string): TimeFrame[] {
    const  re = new RegExp(labelPattern,'i');
    const  output:TimeFrame[] = [];
    timeFrames.forEach((frame, key)=>{
...
export function  getTimeFramesString(labelPattern: string): string {
...
(globalThis as any).isoTimeFormat = /([0-9]{4})(-[0-9]{1,2})?(-[0-9]{1,2})?( |T)?([0-9]{1,2}:[0-9]{2})?(:[0-9]{2})?(.[0-9]*)?(((\+|-)[0-9]:[0-9])|Z)?/;
(globalThis as any).maxDayOfMonth = [0,31,29,31,30,31,30,31,31,30,31,30,31];
