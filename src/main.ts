...
function  getKeywordMatchingScore(targetStrings: string[], keywordsParticples: Particples, thesaurus: Thesaurus): FoundLine {
    // Example:
            ...
            (bestFound: FoundLine, aTargetString: string, stringIndex: number) => {
                ...
                for (let wordIndex = 0;  wordIndex < keywordsParticples.words.length;  wordIndex += 1) {
                    ...
                    if (useThesaurus) {
                        ...
                        const  normalizedResult = getSubMatchedScore(normalizedTargetKeywords, normalizedTargetKeywordsLowerCase,
                            normalizedWord, stringIndex, wordIndex, targetType);
    ...
    function  getSubMatchedScore(targetString: string, targetStringLowerCase: string, keywordParticples: ParticpleWord,
            stringIndex: number, wordIndex: number, targetType: SearchTargetType): Result {
        var  score = 0;
...
function  compareScoreAndSoOnDebug(a: FoundLine, b: FoundLine): number {
    ...
    if (a.searchKeywordCount === 0  ||  b.searchKeywordCount === 0) {
        ...
        if (isDebug && different) { lib.pp(`searchKeywordCount: ${a.searchKeywordCount}, ${b.searchKeywordCount}`); }
        return  different;
    }
    if (a.searchKeywordCount < b.searchKeywordCount) {
        var  kPoint = b.searchKeywordCount - a.searchKeywordCount + 0.5;
    ...
    if (different === 0) {
        different = a.matchedSearchKeywordCount - b.matchedSearchKeywordCount + kPoint;
        if (isDebug && different) { lib.pp(`matchedSearchKeywordCount: ${a.matchedSearchKeywordCount}, ${b.matchedSearchKeywordCount}`); }
    }
    ...
    if (different === 0) {
        ...
        if (isDebug && different) { lib.pp(`partMatchedTargetKeywordCount(=partMatchedCountsByWord.length): ${a.partMatchedTargetKeywordCount}, ${b.partMatchedTargetKeywordCount}`); }
    }
    ...
    if (different === 0) {
        if (a.tagLabel !== b.tagLabel) {
            ...
            if (isDebug && different) { lib.pp(`tagLabel: ${a.tagLabel}, ${b.tagLabel}`); }
        }
    }
    ...
    if (different === 0) {
        different =
            a.caseIgnoredParticipleMatchedTargetKeywordCount -
            b.caseIgnoredParticipleMatchedTargetKeywordCount + kPoint;
        if (isDebug && different) { lib.pp(`caseIgnoredParticipleMatchedTargetKeywordCount(=caseIgnoredParticipleMatchedCountsByWord.length): ${a.caseIgnoredParticipleMatchedTargetKeywordCount}, ${b.caseIgnoredParticipleMatchedTargetKeywordCount}`); }
    }

    // targetWordCount (targetWordCount)
    if (different === 0) {
        different = b.targetWordCount - a.targetWordCount - kPoint;
        if (isDebug && different) { lib.pp(`targetWordCount: ${a.targetWordCount}, ${b.targetWordCount}`); }
    }
    ...
    if (different === 0) {
        different =
            a.participleMatchedTargetKeywordCount -
            b.participleMatchedTargetKeywordCount + kPoint;
        if (isDebug && different) { lib.pp(`participleMatchedTargetKeywordCount(=participleMatchedCountsByWord.length): ${a.participleMatchedTargetKeywordCount}, ${b.participleMatchedTargetKeywordCount}`); }
    }
    ...
    if (different === 0) {
        different =
            a.caseIgnoredSuperMatchedTargetKeywordCount -
            b.caseIgnoredSuperMatchedTargetKeywordCount + kPoint;
        if (isDebug && different) { lib.pp(`caseIgnoredSuperMatchedTargetKeywordCount(=caseIgnoredSuperMatchedCountsByWord.length): ${a.caseIgnoredSuperMatchedTargetKeywordCount}, ${b.caseIgnoredSuperMatchedTargetKeywordCount}`); }
    }

    // superMatchedTargetKeywordCount (superMatchedTargetCount)
    if (different === 0) {
        different = a.superMatchedTargetKeywordCount - b.superMatchedTargetKeywordCount + kPoint;
        if (isDebug && different) { lib.pp(`superMatchedTargetKeywordCount(=superMatchedCountsByWord.length): ${a.superMatchedTargetKeywordCount}, ${b.superMatchedTargetKeywordCount}`); }
    }

    // semiMatchedTargetKeywordCount (semi) (2)
    if (different === 0) {
        different = a.caseIgnoredSemiMatchedTargetKeywordCount - b.caseIgnoredSemiMatchedTargetKeywordCount + kPoint;
        if (isDebug && different) { lib.pp(`caseIgnoredSemiMatchedTargetKeywordCount(=semiMatchedCountsByWord.length): ${a.caseIgnoredSemiMatchedTargetKeywordCount}, ${b.caseIgnoredSemiMatchedTargetKeywordCount}`); }
    }
    ...
    if (different === 0) {
        for (var i = 0; i < a.parentMatchedCount.length; i+=1) {
            different = a.parentMatchedCount[i] - b.parentMatchedCount[i] + kPoint;
            if (different !== 0) {
                if (isDebug && different) { lib.pp(`parentMatchedCount[${i}]: ${a.parentMatchedCount[i]}, ${b.parentMatchedCount[i]}`); }
                break;
            }
        }
    }
    ...
    if (different === 0) {
        var  different = a.score - b.score + kPoint;
        if (isDebug && different) { lib.pp(`score: ${a.score}, ${b.score}`); }
    }
    ...
    if (different === 0) {
        ...
        if (isDebug && different) { lib.pp(`path: ${a.path}, ${b.path}`); }
    }

    // lineNum
    if (different === 0) {
        different = b.lineNum - a.lineNum;
        if (isDebug && different) { lib.pp(`lineNum: ${a.lineNum}, ${b.lineNum}`); }
    }
...
type SearchTargetType = '' | 'normalized' | 'withoutTags';  // '' = with tags
