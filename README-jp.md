...
- [typrm](#typrm)
  - [スニペット検索機能 (#keyword タグ, #glossary タグ) - 精度よくスニペットを検索します](#スニペット検索機能-keyword-タグ-glossary-タグ---精度よくスニペットを検索します)
    ...
    - [かなモードのまま検索](#かなモードのまま検索)
    - [search コマンドの詳細](#search-コマンドの詳細)
    - [typrm タグ内エスケープ](#typrm-タグ内エスケープ)
    - [#glossary タグ](#glossary-タグ)
    - [シソーラス ファイル](#シソーラス-ファイル)
  ...
  - [#alarm タグ - 指定した日時を過ぎたら検索時に表示します](#alarm-タグ---指定した日時を過ぎたら検索時に表示します)
...
検索対象のファイルの内容を全文検索します。
ただし、前述の通り `#keyword:` タグがある内容が優先されて表示されます。
...
typrm の検索機能には、日本語変換が「かなモード」のままで入力しても英語で検索できる
ユニークかつ便利な機能があります。

    $ typrm
    typrm keyword: ぎｔ
    keyword="git"
    .../git.txt:1: #keyword: git

かなモードをオフにしなくても、キーを押したとおりのアルファベットで検索できます。
上記の例の場合、`g i t` とキー入力すると `ぎｔ` と表示されますが、`git` を検索します。

この機能がない場合、現在のかなモードがオンである場合だけ、
かなモードをオフにしてから入力するというややこしい操作をしなければならないか、
入力してからアルファベットに変換する ファンクション キー を押すために
ホーム ポジション から指を離さなければならないため、思考の邪魔になってしまいます。
しかし、この機能があれば、入力が完了したつもりで Enter キーを押しても実は
変換の確定だったと気づいたら、そのままもう一度 Enter キーを押すだけで検索が実行できます。

入力したキーワードに 1文字でも全角のアルファベットがあった場合、
自動的に半角のアルファベットに変換して検索します。
もしすべてひらがなだった場合（たとえば、たけ = take）、
↑ キー を押して入力履歴を表示させて末尾に空白を入力すると
半角のアルファベットに変換して検索します。
もし スペース キー を押して変換候補を表示してしまった場合は、
Esc キー を押してひらがなに戻してください。
Windows では 2回 Esc キー を押してください。
もし変換を確定してしまったときは、残念ながらひらがな以外の文字はアルファベットに変換できません。
複数の単語を入力するときは、単語の区切りで Enter キー と スペース キー を押してください。
...
- `#keyword:` にマッチした行から、上方向にインデントが浅くなる行を探し、
  その浅くなった行に `#keyword:` にマッチしなかった単語を含む場合、上位にヒットします。
  最終的にインデントが無くなるまで上方向に探します。
  ただし、1行目も検索対象になります。
  この処理は、YAML では親マッピング（オブジェクト）の行の内容も含めて検索することに相当します。
  浅くなった行の検索は `#keyword:` タグの有無に関わらず同じスコアで検索します
...
` #`（空白と#）または `"%` を含むキーワードを指定するときは、[typrm タグ内エスケープ](#typrm タグ内エスケープ)をしてください。
...
そうしなければ ` #`（空白と#）は次のタグとして解釈されます。

`"%` を含むキーワードを指定するときは、`""%25"` と書いてください。
そうしなければ、typrm タグ内エスケープとして解釈されます。

検索結果にはエスケープ前（空白と#）に変えて表示されます。 
...
シソーラスには、同義語と包含関係だけ指定できます。
シソーラス ファイル を保存するだけですぐに使われます。
...
**イディオム**のそれぞれの語を空白で分けずに検索キーワードに入力しても、語に分けて検索します。
...
**包含関係**があるときは、サブセットにあたるキーワードを 2列目以降に指定することができます。
たとえば、TypeScript は JavaScript のスーパーセットなので、以下のように設定します。
...
この設定を保存すると、TypeScript array で検索したときでも JavaScript array がヒットして表示されます。
TypeScript array も検索対象です。
...
JavaScript array で検索しても TypeScript array はヒットしません
...
以下のように設定すると、ts という略語を検索キーワードとして入力したとき、
JavaScript や js も検索対象になります。
...
上記の TypeScript と ts は包含関係ではなく同義語の関係です。
上記の TypeScript と JavaScript は包含関係です。
上記の ts と js も包含関係です。

下記のように包含関係のキーワードが別の行の 1列目に存在しなければ、包含関係にはなりません。
下記の TypeScript と JavaScript に関係はありません。
...
上記の js が JavaScript と同義語の関係にあるか TypeScript と同義語の関係にあるかは、
決まっていません。
...
## #alarm タグ - 指定した日時を過ぎたら検索時に表示します

`#alarm:` に続けて日時を書くと、その日時を過ぎていたら、
どのキーワードで検索しても検索結果に表示されます。

    トークンの期限（サンプル）:  #alarm: 2030-12-15

日時は ISO 形式で書きます。タイムゾーンは typrm を実行しているプロセスのタイムゾーンと同じです。

    #alarm: 2020-12-15        #// 2020-12-15 0:00 と同じです
...
- Visual Studio Code >> Terminal >> New Terminal >>（＋の左の 1:__shell__）>> JavaScript Debug Terminal
- npm test
